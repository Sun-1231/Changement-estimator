...
import sys
sys.path.append('/home/fengyuwei/codes/syndrome_decoding_estimator/sd_estimator')
from test3 import *
import all
from prettytable import PrettyTable
...
def sd_estimate_display(n, k, w, memory_limit=inf, bit_complexities=1, hmap=1, skip=["BJMM-dw"], precision=1,
                        truncate=0,
                        all_parameters=0, theoretical_estimates=1, use_mo=1, workfactor_accuracy=1, limit_depth=0,
                        quantum_estimates=0,
...
def _sd_estimate(n, k, w, theoretical_estimates, memory_limit, bit_complexities, hmap, skip, use_mo,
    ...
    if "BM" not in skip and "Both-May" not in skip:
        ...
        bar.next()
    if "BM+" not in skip:
        complexities["BM+"] = {"time": 0, "memory": 0, "parameters": 1.0}
...
def _add_theoretical_estimates(complexities, n, k, w, memory_limit=inf, skip=["BJMM-dw"], use_mo=1, workfactor_accuracy=1):
    code_n = n
    code_k = k
    code_w = w
    ...
    if "BM" not in skip and "Both-May" not in skip:
        ...
        bar.next()
    # if "BM+" not in skip:
    #     tt, mm = bm_time_memory(code_n, code_k, code_w)
    #
    #     complexities["BM+"]["Workfactor time"] = tt
    #     complexities["BM+"]["Workfactor memory"] = mm
    #     bar.next()
...
def List_rate_prange( kk,memory_limit=inf, skip=["BJMM-dw"], use_mo=1, workfactor_accuracy=1):

    grid_std_accuracy = {"prange": [20, 150], "stern": [20, 150], "dumer": [20, 150], "ball_collision": [15, 150],
                         "bjmm": [10, 250], "may-ozerov": [5, 1000], "both-may": [5, 1000]}

    nr_algorithms = 18 - len(skip)
    nr_algorithms += 1 if "BJMM-dw" in skip else 0
    nr_algorithms += 1 if "BJMM-p-dw" in skip or "BJMM-pdw" in skip else 0
    bar = Bar('Computing theoretical workfactors\t', max=nr_algorithms)
    ...
    while rate <1:
        ...
        if "prange" not in skip:
            prangeT,prangeM = prange_workfactor(rate, omega, grid_std_accuracy["prange"][0], grid_std_accuracy["prange"][1],memory_limit)

            # complexities["Prange"]["Workfactor time"] = T
            # complexities["Prange"]["Workfactor memory"] = M
            bar.next()
        # if "stern" not in skip:
        #     sternT, sternM = stern_workfactor(rate, omega, grid_std_accuracy["stern"][0], grid_std_accuracy["stern"][1], memory_limit)
        #
        #     # complexities["Stern"]["Workfactor time"] = T
        #     # complexities["Stern"]["Workfactor memory"] = M
        #     bar.next()
        # if "dumer" not in skip:
        #     dumerT, dumerM = dumer_workfactor(rate, omega, grid_std_accuracy["dumer"][0], grid_std_accuracy["dumer"][1], memory_limit)
        #
        #     # complexities["Dumer"]["Workfactor time"] = T
        #     # complexities["Dumer"]["Workfactor memory"] = M
        #     bar.next()
        # if "ball_collision" not in skip:
        #     ball_collisionT, ball_collisionM = ball_collision_workfactor(rate, omega, grid_std_accuracy["ball_collision"][0],
        #                                      grid_std_accuracy["ball_collision"][1], memory_limit)
        #
        #     # complexities["Ball Collision"]["Workfactor time"] = T
        #     # complexities["Ball Collision"]["Workfactor memory"] = M
        #     bar.next()
        # if "BJMM" not in skip and "MMT" not in skip:
        #     BJMMT,BJMMM = bjmm_workfactor(rate, omega, grid_std_accuracy["bjmm"][0], grid_std_accuracy["bjmm"][1], memory_limit)
        #
        #     # complexities["BJMM (MMT)"]["Workfactor time"] = T
        #     # complexities["BJMM (MMT)"]["Workfactor memory"] = M
        #     bar.next()
        # if "MO" not in skip and "May-Ozerov" not in skip:
        #     MOT, MOM = may_ozerov_workfactor(rate, omega, grid_std_accuracy["may-ozerov"][0],
        #                                  grid_std_accuracy["may-ozerov"][1], memory_limit, use_mo)
        #
        #     # complexities["May-Ozerov"]["Workfactor time"] = T
        #     # complexities["May-Ozerov"]["Workfactor memory"] = M
        #     bar.next()
        # if "BM" not in skip and "Both-May" not in skip:
        #     BMT, BMM = both_may_workfactor(rate, omega, grid_std_accuracy["both-may"][0], grid_std_accuracy["both-may"][1],
        #                                memory_limit, use_mo)
        #
        #     # complexities["Both-May"]["Workfactor time"] = T
        #     # complexities["Both-May"]["Workfactor memory"] = M
        #     bar.next()
        # if "BM+" not in skip:
        #     BM_plusT, BM_plusM = bm_time_memory(code_n, code_k, code_w)
        #
        #
        #     # complexities["BM+"]["Workfactor time"] = tt
        #     # complexities["BM+"]["Workfactor memory"] = mm
        #     bar.next()
        List_prange_rate.append('%.4f' %rate)
        List_prange_time.append('%.4f'%prangeT)
        List_prange_memory.append('%.4f'%prangeM)
        List_prange_omega.append('%.4f'%omega)
        rate += 0.05
        #print("ccccccccccc")

    new_List_prange_rate = []
    new_List_prange_time = []
    new_List_prange_memory = []
    new_List_prange_omega = []

    for n in List_prange_time:
        new_List_prange_time.append(float(n))
    List_prange_time = new_List_prange_time

    for n in List_prange_rate:
        new_List_prange_rate.append(float(n))
    List_prange_rate = new_List_prange_rate

    for n in List_prange_memory:
        new_List_prange_memory.append(float(n))
    List_prange_memory = new_List_prange_memory

    for n in List_prange_omega:
        new_List_prange_omega.append(float(n))
    List_prange_omega = new_List_prange_omega

    print("output\n")
    # draw a table
    pt=PrettyTable()
    pt.field_names=["List_prange_rate","List_prange_time","List_prange_memory"]
    
    for item1,item2,item3 in zip(List_prange_rate,List_prange_time,List_prange_memory):
        pt.add_row([item1,item2,item3])
    print(pt)
    
    print("List_prange_rate=",List_prange_rate)
    print("List_prange_time=",List_prange_time)
    print("List_prange_memory=",List_prange_memory)
    #0510 print(List_prange_omega)


    # print("workfactor:")
    # x = PrettyTable()
    # x.title = '2^c*n  (c的值)'
    # x.field_names = ["algorithm", "time", "memory"]
    #
    # x.add_row(["prange", '%.4f'%prangeT, '%.4f'%prangeM,])
    # x.add_row(["stern", '%.4f'%sternT, '%.4f'%sternM])
    # x.add_row(["dumer", '%.4f'%dumerT, '%.4f'%dumerM])
    # x.add_row(["ball_collision", '%.4f'%ball_collisionT, '%.4f'%ball_collisionM])
    # x.add_row(["BJMM", '%.4f'%BJMMT, '%.4f'%BJMMM])
    # x.add_row(["MO", '%.4f'%MOT, '%.4f'%MOM])
    # x.add_row(["BM", '%.4f'%BMT, '%.4f'%BMM])
    # x.add_row(["BM_Plus", '%.4f'%BM_plusT, '%.4f'%BM_plusM])
    # print(x)
    #
    # bar.finish()

def List_rate_dumer( kk,memory_limit=inf, skip=["BJMM-dw"], use_mo=1, workfactor_accuracy=1):
    ...
    while rate <1:
        ...
        # if "prange" not in skip:
        #     prangeT,prangeM = prange_workfactor(rate, omega, grid_std_accuracy["prange"][0], grid_std_accuracy["prange"][1],
        #                              memory_limit)
        #
        #     # complexities["Prange"]["Workfactor time"] = T
        #     # complexities["Prange"]["Workfactor memory"] = M
        ...
        #     # complexities["BM+"]["Workfactor memory"] = mm
        #     bar.next()
        List_prange_time.append('%.4f'%dumerT)
        List_prange_memory.append('%.4f'%dumerM)
        rate += 0.05
    ...
    List_prange_memory = new_List_prange_memory
    print("output\n")
    print(List_prange_time)
    print(List_prange_memory)
...
def List_rate_stern( kk,memory_limit=inf, skip=["BJMM-dw"], use_mo=1, workfactor_accuracy=1):

    grid_std_accuracy = {"prange": [20, 150], "stern": [20, 150], "dumer": [20, 150], "ball_collision": [15, 150],
                         "bjmm": [10, 250], "may-ozerov": [5, 1000], "both-may": [5, 1000]}

    nr_algorithms = 20 - len(skip)
    nr_algorithms += 1 if "BJMM-dw" in skip else 0
    nr_algorithms += 1 if "BJMM-p-dw" in skip or "BJMM-pdw" in skip else 0
    bar = Bar('Computing theoretical workfactors\t', max=nr_algorithms)
    ...
    while rate <1:
        ...
        #     bar.next()
        List_prange_rate.append('%.4f'%rate)
        List_prange_time.append('%.4f'%sternT)
        List_prange_memory.append('%.4f'%sternM)
        rate += 0.05
    ...
    # draw a table
    pt=PrettyTable()
    pt.field_names=["List_stern_rate","List_stern_time","List_stern_memory"]
...
def List_rate_BJMM( kk,memory_limit=inf, skip=["BJMM-dw"], use_mo=1, workfactor_accuracy=1):
    ...
    while rate <1:
        ...
        #     # complexities["BM+"]["Workfactor memory"] = mm
        #     bar.next()
        List_prange_time.append('%.4f'%BJMMT)
        List_prange_memory.append('%.4f'%BJMMM)
        rate += 0.05
...
def List_rate_MO( kk,memory_limit=inf, skip=["BJMM-dw"], use_mo=1, workfactor_accuracy=1):
    ...
    while rate <1:
        ...
        #     # complexities["BM+"]["Workfactor memory"] = mm
        #     bar.next()
        List_prange_time.append('%.4f'%MOT)
        List_prange_memory.append('%.4f'%MOM)
        rate += 0.05
...
def List_rate_BM( kk,memory_limit=inf, skip=["BJMM-dw"], use_mo=1, workfactor_accuracy=1):
    ...
    while rate <1:
        ...
        #     # complexities["BM+"]["Workfactor memory"] = mm
        #     bar.next()
        List_prange_time.append('%.4f'%BMT)
        List_prange_memory.append('%.4f'%BMM)
        rate += 0.05
    ...
    while rate <1:
        ...
        #     # complexities["BM+"]["Workfactor memory"] = mm
        #     bar.next()
        List_prange_rate.append('%.2f'%rate)
        List_prange_time.append('%.4f'%prangeT)
        List_prange_memory.append('%.4f'%prangeM)
        rate += 0.01
    new_List_prange_rate = []   
